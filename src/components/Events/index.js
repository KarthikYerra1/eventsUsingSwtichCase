...
class Events extends Component {
  ...
  getActiveRegistrationStatus = () => {
    ...
    const activeEvent = eventsList.find(each => each.id === selectedEventId)
          ...
          <ul className="event-cards-container">
            {eventsList.map(eachEvent => (
              <EventItem
                key={eachEvent.id}
                details={eachEvent}
                selectEventId={this.selectEventId}
                isSelected={eachEvent.id === selectedEventId}
              />
            ))}
          </ul>
        </div>
        <ActiveEventRegistrationDetails
          eventRegistrationStatus={this.getActiveRegistrationStatus()}
        />
      </div>
    )
  }
